...
export default function Refund() {
  ...
  const toggleSelect = (emp) => {
    ...
    if (selected.some((x) => x.eid === emp.eid)) {
      updated = selected.filter((x) => x.eid !== emp.eid);
    } else {
  ...
  return (
    ...
    >
      {/* ================= TABS ================= */}
      <div className="flex border-b border-gray-200 bg-white text-sm sm:text-base rounded-t-xl">
        <button
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#f5f6ff] text-[#2E3092] border-b">
                    ...
                    <th className="p-3 text-left">{t("refund_table_name")}</th>
                    <th className="p-3 text-left">{t("refund_table_eid")}</th>
                    <th className="p-3 text-left">{t("refund_table_salary")}</th>
                    <th className="p-3 text-left">{t("refund_table_payment_date")}</th>
                  </tr>
                </thead>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
            <div className="grid md:grid-cols-4 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  placeholder={t("refund_search_placeholder")}
              ...
              <div className="relative">
                ...
                <input
                  placeholder={t("refund_filter_payment_date")}
              ...
              >
                <option value="all">{t("refund_status_all")}</option>
                <option value="done">{t("refund_status_done")}</option>
                <option value="failed">{t("refund_status_failed")}</option>
              </select>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
              ...
              <table className="w-full text-sm min-w-[700px]">
                <thead>
                  <tr className="bg-[#f5f6ff] text-[#2E3092] border-b">
                    ...
                    <th className="p-3 text-left">{t("refund_table_salary")}</th>
                    <th className="p-3 text-left">{t("refund_table_payment_date")}</th>
                    <th className="p-3 text-left">{t("refund_table_request_date")}</th>
                    <th className="p-3 text-left">{t("refund_table_status")}</th>
                    <th className="p-3 text-left">{t("refund_table_desc")}</th>
                  </tr>
                </thead>
            ...
            <div className="flex items-center justify-between mt-4 text-sm">
              <span className="text-gray-500">
                {t("refund_showing")} {paginated.length} {t("refund_of")}{" "}
                {filteredRefunds.length} {t("refund_records")}
              </span>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-gray-200 p-6">
              ...
              {selected.map((emp, idx) => (
                ...
                >
                  ...
                  <div className="flex justify-between text-xs text-gray-500 mt-1">
                    <span>{t("refund_salary")}: AED {emp.salary}</span>
                    <span>{t("refund_payment_date")}: {emp.paymentDate}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-gray-100 border rounded-xl p-3 mb-6">
              <p className="text-xs text-gray-500">{t("refund_final_amount")}</p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  toast.success(t("refund_submitted"));
                  setSelected([]);
                  setSelectAll(false);
                }}
                className="px-5 py-2 bg-[#2E3092] hover:bg-[#23246e] text-white rounded-md text-sm flex items-center gap-2 transition"
